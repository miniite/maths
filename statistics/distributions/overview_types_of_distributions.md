...
| **Distribution** | **Type** | **Random Variable** | **Probability Function** | **Parameters** | **Shape** | **Applications** |
|------------------|----------|---------------------|--------------------------|----------------|-----------|------------------|
| **Bernoulli** | Discrete | Binary (0 or 1) | PMF: $ P(X = x) = p^x (1-p)^{1-x} $, $ x \in \{0, 1\} $ | $ p $: Probability of success ($ 0 \leq p \leq 1 $) | Two bars (binary outcomes) | Binary classification (e.g., churn prediction), A/B testing, click-through rates |
| **Binomial** | Discrete | Count of successes ($ 0, 1, \ldots, n $) | PMF: $ P(X = k) = \binom{n}{k} p^k (1-p)^{n-k} $, $ k = 0, 1, \ldots, n $ | $ n $: Number of trials, $ p $: Probability of success | Skewed (depends on $ p $) or symmetric (if $ p = 0.5 $) | Modeling successes in trials (e.g., defective items, customer conversions) |
| **Normal (Gaussian)** | Continuous | Real numbers ($ -\infty < x < \infty $) | PDF: $ f(x) = \frac{1}{\sigma \sqrt{2\pi}} e^{-\frac{(x - \mu)^2}{2\sigma^2}} $ | $ \mu $: Mean, $ \sigma $: Standard deviation | Symmetric, bell-shaped | Modeling symmetric data (e.g., heights, test scores), regression assumptions |
| **Poisson** | Discrete | Non-negative integers ($ 0, 1, 2, \ldots $) | PMF: $ P(X = k) = \frac{\lambda^k e^{-\lambda}}{k!} $, $ k = 0, 1, \ldots $ | $ \lambda $: Average rate of occurrence | Right-skewed (for small $ \lambda $) | Event counts (e.g., customer arrivals, website visits, defects per unit) |
| **Log-Normal** | Continuous | Positive real numbers ($ x > 0 $) | PDF: $ f(x) = \frac{1}{x \sigma \sqrt{2\pi}} e^{-\frac{(\ln x - \mu)^2}{2\sigma^2}} $ | $ \mu $: Mean of logarithm, $ \sigma $: Standard deviation of logarithm | Right-skewed | Modeling skewed data (e.g., incomes, time-to-failure, stock prices) |
| **Uniform** | Discrete or Continuous | Discrete: Finite set; Continuous: Interval $[a, b]$ | Discrete PMF: $ P(X = x) = \frac{1}{n} $; Continuous PDF: $ f(x) = \frac{1}{b-a} $, $ a \leq x \leq b $ | Discrete: $ n $ (number of outcomes); Continuous: $ a $, $ b $ (interval bounds) | Flat (equal probability) | Random sampling (e.g., lottery numbers, simulation inputs) |



| **Distribution** | **Function** | **Mean** | **Mode** | **Median** | **Variance** | **Standard Deviation** | **Skewness** |
|------------------|--------------|----------|----------|------------|--------------|-----------------------|--------------|
| **Bernoulli** | PMF: $ P(X = x) = p^x (1-p)^{1-x} $, $ x \in \{0, 1\} $ | $ p $ | $ 0 $ (if $ p < 0.5 $),<br> <br> $ 1 $ (if $ p > 0.5 $) | $ 0 $ (if $ p < 0.5 $),<br> <br> $ 1 $ (if $ p > 0.5 $),<br> <br> $ 0.5 $ (if $ p = 0.5 $) | $ p(1-p) $ | $ \sqrt{p(1-p)} $ | $ \frac{1 - 2p}{\sqrt{p(1-p)}} $ |
| **Binomial** | PMF: $ P(X = k) = \binom{n}{k} p^k (1-p)^{n-k} $, $ k = 0, 1, \ldots, n $ | $ np $ | $ \lfloor (n+1)p \rfloor $ | Approximately $ np $ for large $ n $ | $ np(1-p) $ | $ \sqrt{np(1-p)} $ | $ \frac{1 - 2p}{\sqrt{np(1-p)}} $ |
| **Normal (Gaussian)** | PDF: $ f(x) = \frac{1}{\sigma \sqrt{2\pi}} e^{-\frac{(x - \mu)^2}{2\sigma^2}} $ | $ \mu $ | $ \mu $ | $ \mu $ | $ \sigma^2 $ | $ \sigma $ | $ 0 $ |
| **Poisson** | PMF: $ P(X = k) = \frac{\lambda^k e^{-\lambda}}{k!} $, $ k = 0, 1, \ldots $ | $ \lambda $ | $ \lfloor \lambda \rfloor $ (if $ \lambda $ is not an integer) or $ \lambda $ or $ \lambda - 1 $ | Approximately $ \lambda $ for large $ \lambda $ | $ \lambda $ | $ \sqrt{\lambda} $ | $ \frac{1}{\sqrt{\lambda}} $ |
| **Log-Normal** | PDF: $ f(x) = \frac{1}{x \sigma \sqrt{2\pi}} e^{-\frac{(\ln x - \mu)^2}{2\sigma^2}} $ | $ e^{\mu + \frac{\sigma^2}{2}} $ | $ e^{\mu - \sigma^2} $ | Not analytically expressible; approximated numerically | $ (e^{\sigma^2} - 1) e^{2\mu + \sigma^2} $ | $ \sqrt{(e^{\sigma^2} - 1) e^{2\mu + \sigma^2}} $ | $ (e^{\sigma^2} + 2) \sqrt{e^{\sigma^2} - 1} $ |
| **Uniform** | Discrete PMF: $ P(X = x) = \frac{1}{n} $; <br> <br> Continuous PDF: $ f(x) = \frac{1}{b-a} $, $ a \leq x \leq b $ | Discrete: $ \frac{x_1 + x_n}{2} $;<br> <br> Continuous: $ \frac{a + b}{2} $ | Any value (discrete); any value in $[a, b]$ (continuous) | Discrete: $ \frac{x_1 + x_n}{2} $;<br> <br> Continuous: $ \frac{a + b}{2} $ | Discrete: $ \frac{n^2 - 1}{12} $;<br> <br> Continuous: $ \frac{(b-a)^2}{12} $ | Discrete: $ \sqrt{\frac{n^2 - 1}{12}} $; <br> <br> Continuous: $ \frac{b-a}{\sqrt{12}} $ | $ 0 $ (for continuous) |
